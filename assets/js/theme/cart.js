...
export default class Cart extends PageManager {
    ...
    qtyUpdate($target) {
        ...
        utils.api.cart.itemUpdate(itemId, newQty, (err, response) => {
            ...
            if (response.data.status === 'succeed') {
                ...
                $el.val(oldQty);
                $(`#qty-${itemId}-dialog`).css('display', 'block'); // eslint-disable-line no-use-before-define
    ...
    refreshContent(remove) {
        ...
        utils.api.cart.getContent(options, (err, response) => {
            ...
            const cart = $('#cartForm');
            if (cart.length > 0) {
                const items = cart.find('[data-sku][data-product]').each(function () { // eslint-disable-line func-names
                    const sku = $(this).attr('data-sku') || $(this).attr('data-product');
                    const qty = $(this).find('.quantityInput').val();
                    const price = $(this).find('.CartItemPrice,.CartItemIndividualPrice').text().trim().substring(1);
                    const title = $(this).find('.ProductName,.ProductDetails').find('a').first().text();
                    const image = $(this).find('.CartThumb,.ProductImage').find('img').first().attr('src');
                    const link = $(this).find('.ProductName,.ProductDetails').find('a').first().attr('href');
                    _ltk.SCA.AddItemWithLinks(sku, qty, price, title, image, link); // eslint-disable-line
                });
                if (items.length === 0) {
                    _ltk.SCA.ClearCart(); // eslint-disable-line
                }
                _ltk.SCA.Submit(); // eslint-disable-line
            }
        });
    }
    ...
    bindCartEvents() {
        ...
        const qtyUpdate = _.bind(_.debounce(this.qtyUpdate, debounceTimeout), this);
        ...
        $('[data-quantity-update]', this.$cartContent).on('change', (event) => {
            const $target = $(event.currentTarget);
